...
===========================
Website: ethyra.xyz       )
Twitter: x.com/ethyra_xyz )
Telegram: t.me/ethyra_xyz )
============================
...
contract Ethyra is ERC20 {
    ...
    constructor(
        ...
        uint256 liquidityAmount = _totalSupply.mul(9675).div(10000);
    ...
    function distributeAirdrop(uint256 totalAirdropAmount) external onlyOwner {
    ...
    function excludeUserFromAirdrop(address account) external onlyOwner {
        for (uint256 i = 0; i < _holders.length; i++) {
            if (_holders[i] == account) {
                _holders[i] = _holders[_holders.length - 1];
                _holders.pop();
