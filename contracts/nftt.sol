...
contract nftContract is ERC721, ERC721URIStorage, Ownable {
    ...
    function wearItems(
        ...
        require(_headTokenId == 0 || !nfts[_headTokenId - 1].isOnSale, "Head on sale, you cannot wear it!");
        require(_headTokenId == 0 || !nfts[_headTokenId - 1].isBiddable, "Head on bid, you cannot wear it!");
        require(_middleTokenId == 0 || !nfts[_middleTokenId - 1].isOnSale, "Middle on sale, you cannot wear it!");
        require(_middleTokenId == 0 || !nfts[_middleTokenId - 1].isBiddable, "Middle on bid, you cannot wear it!");
        require(_bottomTokenId == 0 || !nfts[_bottomTokenId - 1].isOnSale, "Bottom on sale, you cannot wear it!");
        require(_bottomTokenId == 0 || !nfts[_bottomTokenId - 1].isBiddable, "Bottom on bid, you cannot wear it!");
    ...
    function wearItem(uint256 _tokenId) public {
        ...
        require(!nfts[_tokenId - 1].isOnSale, "You cannot wear an item while it is on sale.");
        require(!nfts[_tokenId - 1].isBiddable, "You cannot wear an item while it is on auction.");
    ...
    function putOnSale(uint256 _tokenId, uint256 _sellPrice) public {
        require(ownerOf(_tokenId) == msg.sender, "You cannot put this item on sale, because you are not the owner of it.");
        require(!nfts[_tokenId - 1].isOnSale, "Item is already on sale!");
        require(!nfts[_tokenId - 1].isWearing, "You must unwear it first, then you can sell it.");
    ...
    function cancelSale(uint256 _tokenId) public {
        require(nfts[_tokenId - 1].isOnSale, "Item should be on sale first, to be cancelled.");
    ...
    function buyFromSale(uint256 _tokenId) public payable {
        require(ownerOf(_tokenId) != msg.sender, "You cannot buy your own item!");
        require(nfts[_tokenId - 1].isOnSale, "Item should be on sale for you to buy it.");
        require(nfts[_tokenId - 1].sellPrice <= msg.value, "The amount you tried to buy is less than the price.");
        require(getApproved(_tokenId) == address(this), "Seller did not approve the contract to sell this item.");
        ...
        if (nfts[_tokenId - 1].maxBid > 0) {
            users[nfts[_tokenId - 1].maxBidder].userBalance += nfts[_tokenId - 1].maxBid;
        }
    ...
    function putOnAuction(uint256 _tokenId) public {
        ...
        require(!nfts[_tokenId - 1].isWearing, "You must unwear it first, then you can put it on auction.");
        require(!nfts[_tokenId - 1].isBiddable, "This item is already on auction!");
    ...
    function cancelAuction(uint256 _tokenId) public {
        ...
        require(nfts[_tokenId - 1].isBiddable, "Item must be on auction before it can be canceled.");
    ...
    function bid(uint256 _tokenId) public payable {
        ...
        require(nfts[_tokenId - 1].isBiddable, "Item is not biddable!");
        require(msg.value >= nfts[_tokenId - 1].maxBid, "The amount you tried to bid is less than the current max bid.");
        require(ownerOf(_tokenId) != msg.sender, "You cannot bid on your own item.");
    ...
    function acceptHighestBid(uint256 _tokenId) public {
        ...
        require(nfts[_tokenId - 1].isBiddable, "Item must be biddable to accept its highest bid.");
        ...
        users[msg.sender].userBalance += nfts[_tokenId - 1].maxBid;
        nfts[_tokenId - 1].maxBid = 0; 
        nfts[_tokenId - 1].maxBidder = address(0); 
        nfts[_tokenId - 1].isBiddable = false; 
        nfts[_tokenId - 1].isOnSale = false; 
    ...
    function mint(
        ...
        require(!isExist[_cid], "Item link must be unique to mint.");
        require(nfts.length < maxSupply, "Maximum supply reached.");
        require(_clothType == 1 || _clothType == 2 || _clothType == 3, "Invalid cloth type.");
